...
sql_ctw <- left_join(sql_transect_metadata, POP_weir) %>%
  ...
  group_by(StreamName, POP_NAME, weir, Trap_Year, AboveWeir)
#------------------------------------------------------------------------------
...
hatchery_fraction <- left_join(nat_spawn, hat_spawn) %>%
  select(StreamName, weir, POP_NAME, Trap_Year, natural_spawners, hatchery_spawners)
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
# Calculate Pre-spawn mortality
prespawn_mortality <- sql_ctw %>%
  group_by(StreamName, weir, POP_NAME, Trap_Year) %>%
  filter(Spawned == 'No') %>%
...
f_us <- left_join(f_us_carcass, m_us_carcass) %>%
  group_by(StreamName, weir, POP_NAME, Trap_Year) %>%
  select(StreamName, weir, POP_NAME, Trap_Year, us_female_carcass, 
         us_male_carcass)
#------------------------------------------------------------------------------
...
f_ds <- left_join(f_ds_carcass, m_ds_carcass) %>%
  group_by(StreamName, weir, POP_NAME, Trap_Year) %>%
  select(StreamName, weir, POP_NAME, Trap_Year, ds_female_carcass, ds_male_carcass)
#------------------------------------------------------------------------------
## Join Sex, Origin, & Prespawn Mortality
origin_sex <- full_join(f_us, f_ds)%>%
...
#------------------------------------------------------------------------------
# you could create a select statement to ease up that mess.
#------------------------------------------------------------------------------

# save origin_sex
save(origin_sex, file = './data/origin_sex.Rda')
#------------------------------------------------------------------------------
